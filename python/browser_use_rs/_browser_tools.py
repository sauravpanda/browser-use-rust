...
_UPSTREAM_NAME_ALIASES = {
    ...
    # baseline). The Gemini-3 `pages=N` kwarg-mismatch bug is fixed at
    # DISPATCH time in agent/__init__.py:_run_tool — see the
    # "v0.11.12 smart scroll arg-rewrite" block. This avoids the
    # tool-schema cache invalidation that v0.11.10 introduced when it
    # promoted these to real @tool defs with `pages` kwarg.
