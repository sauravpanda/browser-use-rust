        ...
        self._messages.append(
            UserMessage(
                content=(
                    ...
                    "from the last 30 days. Avoid Google/Bing, `after:` "
                    "queries, and manual search-engine input edits. Try "
                    "Barron's search URL `https://www.barrons.com/search?"
                    "query=value%20investing&duration=30d` once. If it "
                    "does not immediately show article cards, navigate "
                    "directly to `https://duckduckgo.com/?q=site%3A"
                    "barrons.com+%22value+investing%22&df=m&ia=web` and "
                    "extract from that current results page. If "
                    "`extract_result_cards(...)` returns mostly page chrome, "
                    "use `extract_structured_data(...)` on the same results "
                    "page, then scroll at most once and extract again. "
                    "Include only specific Barron's news articles with "
                    "publication dates inside the 30-day window. Exclude "
                    "ticker pages, fund pages, topic pages, and market-data "
                    "pages. Once the visible result cards identify the "
                    "matching article titles and dates, finish; do not keep "
                    "re-querying for perfect coverage."
                )
            )
        )
